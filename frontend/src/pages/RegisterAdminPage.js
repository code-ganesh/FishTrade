// client/src/pages/RegisterAdminPage.js
...
const RegisterAdminPage = () => {
  ...
  const handleSubmit = async (userData) => {
    ...
    try {
      ...
      navigate('/dashboard/admin'); // Redirect admin to their dashboard
    } catch (err) {
      setError(err.message || 'Registration failed. Check admin key and try again.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-sky-blue to-sea-blue flex flex-col items-center justify-center overflow-hidden py-12">
      ...
      <div className="relative z-20">
        <AdminRegisterForm onSubmit={handleSubmit} isLoading={isLoading} error={error} />
      </div>
    </div>
  );
};
